	...
	if (val != "") {
		...
		if (filesize<=fileuploadmaxSize) {
			$.ajax({
				...
				//url: "/document/base?cmd=uploadAffix&fieldCode="+tid+"&batchNo="+$a.val()+"&bs_code="+bs_code+"&isbusiness=1&token="+token,
...
//根据Json初始化详情页面
function initDetailPageByJson($div,dataSourceCode,json,type){
	var detailPageHtml = "";
	$.ajax({
		type : "GET",
		async : false,
		url : "/system/base?initDetailPageByJson",
		dataType : "html",
		data : {
			'showType' : 'INSUP',
	...
	$div.append(detailPageHtml);
	modifyAffixFileInput();
	setLable($div, dataSourceCode);
	if (type=='add') {//新增
		changeAffixShow($div,"add");
	} else {//修改
		changeAffixShow($div,"mod");
	}
}