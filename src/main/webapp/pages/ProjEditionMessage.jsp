...
function showmessage(map,id){
   ...
   $("div[id='message'] iframe").attr("src",message+"="+id+"&time="+time_s); 
...
function showmessagepmo(map,id){
   ...
   $("div[id='message'] iframe").attr("src",message+"="+id+"&type=1"+"&time="+time_s); 
...
function visitSituation(){
	$.ajax({
		url : '/system/project?cmd=show_history&id=<%=request.getParameter("ParentPKValue")%>',
		type : 'post',	
		dataType : "json",
