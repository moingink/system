...
{
	name : '修改',
	fun : 'setUpUpdate(this),updateRow(this)',
	buttonToken : 'updateMDEF'
}, {
	name : '删除',
	fun : 'deleteRowCheck(this)',
	buttonToken : 'deleteMDEF'
}, {
	name : '界面初始化',
	fun : 'initStyle(this)',
	buttonToken : ''
...
function initStyle(t) {
	var url = "/system/style/init?" + "sid=" + sid + "&smate=" + smate
			+ "&smate_b=" + smate_b;
	$.ajax({
		url : url,
		async : false
...
function initDetailTable(parentId) {
	console.log('主子表');
	initDetailTableList();
	bulidListPage($("#detailTable"), 'BS_MD_STYLE_B', pageParamFormat("PID = '"
			+ parentId + "'"));
	initDetailDiv();
	bulidMaintainPage($('#detailDiv'), "BS_MD_STYLE_B", '');
	// dateBoxHandle();
	validJsonDetail = transToServer(findUrlParam('base', 'queryValids',
			'&dataSourceCode=BS_MD_STYLE_B'), '');
	$('#detailDiv').bootstrapValidator(validJsonDetail);
}
...
function updateDetail() {
	var selected = JSON.parse(JSON.stringify($('#detailTable').bootstrapTable(
			'getSelections')));
	if (selected.length != 1) {
...
function deleteDetail() {
	...
	var tableData = JSON.parse(JSON.stringify($('#detailTable').bootstrapTable(
			"getData")));
...
function detailSave() {
	...
	if (json['ID'] == "") {
		...
		$('#detailTable').bootstrapTable('updateByUniqueId', {
			id : json['ID'],
			row : json
		});
	}
...
function savaByQuery(t, _dataSourceCode, $div) {
	...
	var message = transToServer(findBusUrlByButtonTonken(buttonToken, '',
			_dataSourceCode), getJson($('#insPage')), childJsonData,
			"BS_MD_STYLE_B", deleteIds);
...
function transToServer(url, jsonData, childJsonData, childDataSourceCode,
		deleteIds) {
	var message;
	$.ajax({
		async : false,
		...
		error : function(XMLHttpRequest, textStatus, errorThrown) {
			...
			if (XMLHttpRequest.getResponseHeader("TIMEOUTURL") != null) {
				window.top.location.href = XMLHttpRequest
						.getResponseHeader("TIMEOUTURL");
			}
			message = "请求失败";
		}
	});
...
function setUpDetailTable() {
	$("#detailDiv")
			.find(
					"#CONTRACT_CODE,#CONTRACT_NAME,#CONTRACT_SIGNATORY,#CONTRACT_EFFECT_DATE,#BUSINESS_TYPE,#PRODUCT_MOLD,#CONTRACT_TAX,#CONTRACT_PERIOD,#PRODUCT_TYPE")
			.attr("readonly", "true");
	$("#detailDiv").append(
			"<input hidden='hidden' type='text' id='DETAIL_TYPE' value='0'>");
}
// 修改
...
// 重写 查询 覆盖crud.js里面的查询
function queryTable(t) {
	if (sid != null) {
		var param = "&SEARCH-SID=" + sid;
	}
