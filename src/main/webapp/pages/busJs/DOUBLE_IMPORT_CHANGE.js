buttonJson =[
              ...
              {name:'新增',fun:'tog(this),addTable(this)',buttonToken:'addDoubleImportChange'},
              {name:'修改',fun:'updateRow(this),updateTable(this)',buttonToken:'updateDoubleImportChange'},
              {name:'删除',fun:'delRows(this)',buttonToken:'deleteDoubleImportChange'}
			];

var childDataSourceCode = "DOUBLE_IMPORT_CHANGE_DETAIL";
var validJsonDetail = {};
...
function write($write, writeMessage, colName, dataSourceCode) {
	if ($write.length > 0) {
		...
		if(dataSourceCode == 'DOUBLE_IMPORT_REF'){
			var DOUBLE_IMPORT_ID = $('#DOUBLE_IMPORT_ID').val();
			if(DOUBLE_IMPORT_ID != ""){
				buildChildTable();
				bulidListPage($("#detailTable"),'DOUBLE_IMPORT_DETAIL',pageParamFormat("PARENT_ID = '"+DOUBLE_IMPORT_ID+"' "));
			}
		}
	} else {
		$("#" + colName).val(writeMessage);
	}
	validatorRef(dataSourceCode,colName);
}
...
function savaByQuery(t,_dataSourceCode,$div){
	...
	var nowTotalAdValorem = 0;
	$.each(rowsData, function (i){
       	childJsonData.push(JSON.stringify(rowsData[i]));
		var adValorem = rowsData[i]['AD_VALOREM'];
       	nowTotalAdValorem = Math.round((Number(nowTotalAdValorem)+Number(adValorem.replace(",","")))* 100) / 100;
    });
    var defaultTotalAdValorem = $('#TOTAL_AD_VALOREM').val();
    if(nowTotalAdValorem != defaultTotalAdValorem){
    	oTable.showModal('提示', "当前单据价税合计总金额与原单据价税合计总金额不相等!");
		return;
    }
...
var dbclickHtml = '<div class="modal-body" style="overflow:hidden;" id="detail">'+
						'<div id="detailTableDbclick" style="padding: 0;"></div>'+
				  '</div>'+
				  '<div class="modal-body" style="overflow:hidden;" id="business">'+
						'<div id="businessTableDbclick" style="padding: 0;"></div>'+
				  '</div>';
...
function show_dbclick(selected,title){
	...
	$('#bus_message_dbclick').parent().next().remove();
	if($('#detail').length == 0 && $('#business').length == 0){
	...
	}else if(selected.hasOwnProperty('PERIOD_OF')){
		initDetailPageByJson($("#bus_message_dbclick"),"FIN_BUSINESS_IMPORT",JSON.stringify(selected));
	}else{
		initDetailPageByJson($("#bus_message_dbclick"),dataSourceCode,JSON.stringify(selected));
		var matchNoArray = selected['MATCH_NO'].split('@');
		var param = "";
		if(matchNoArray[0] != ""){
			param = " MATCH_NO __turn__LIKE '"+matchNoArray[0]+"%25' ";
		}else{
			param = " MATCH_NO __turn__LIKE '%40%25' ";
		}
		bulidListPage($("#detailTableDbclick"),childDataSourceCode,pageParamFormat("PARENT_ID = '"+selected['ID']+"' "));
		bulidListPage($("#businessTableDbclick"),'FIN_BUSINESS_IMPORT_DOUBLE_IMPORT_CHANGE_REF',pageParamFormat(param));
	}
...
//表单处理
function formHandle(){
	$('#BILL_NO').next().css('display','none');//报账单号
	$('#BILL_NO').attr('size','40');
}
...
function buildChildTable(){
	if($('#insPage').children('.col-md-12').length == 0){
		...
		validJsonDetail = transToServer(findUrlParam('base','queryValids','&dataSourceCode=DOUBLE_IMPORT_CHANGE_DETAIL'),"","","","");
...
function ref_end_code(dataSourceCode,rejsonArray) {
	...
	for (var i = 0; i < mappingArray.length; i++) {
		...
		if (writeMessage.length > 0 || writeMessage == '') {
...
function findPageParamByDataSourceCode(_dataSourceCode) {
	...
	if(_dataSourceCode != "DOUBLE_IMPORT_CHANGE_DETAIL" && _dataSourceCode != "DOUBLE_IMPORT_DETAIL"){
		//组装查询参数
