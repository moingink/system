...
<html lang="zh-cn">
	...
	<body id="body_div">
	<div id="tip" style="display: none"><p style="color: red;text-align: right">Tip:如项目金额超过300万元，请上传项目可研方案和第三方评审意见！</p></div>
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<div>
								...
								<!-- <button id="back" type="button" class="btn btn-default" onclick="javascript:history.back(-1);">
									<span class="glyphicon glyphicon-saved" aria-hidden="true"></span>返回
								</button>
								<button id="fh" type="button" class="btn btn-default" onclick="back(this)"  style="visibility: hidden">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			if(pageCode=="PROJ_REQUIREMENT"){
				...
				ajaxPre();
				//$('#users').append('<tr><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="busiType" name="costtype" style="width:90%"><option  selected="selected">请选择</option><option value="1">主设备费用</option><option value="2">软件开发</option><option value="3">安装工程费</option><option value="4">工程建设其他费</option><option value="5">预备费</option><option value="6">贷款利息</option></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="busiType" name="costtype" style="width:90%"><option  selected="selected">请选择</option><option value="1">主设备费用</option><option value="2">软件开发</option><option value="3">安装工程费</option><option value="4">工程建设其他费</option><option value="5">预备费</option><option value="6">贷款利息</option></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="productName" name="costtype" style="width:90%"><option selected="selected">请选择</option><option value="1">主设备费用</option><option value="2">软件开发</option><option value="3">安装工程费</option><option value="4">工程建设其他费</option><option value="5">预备费</option><option value="6">贷款利息</option></select></td><td style="text-align: center; vertical-align: middle; ">111</td><td style="text-align: center; vertical-align: middle; "><input type="button"  onclick="del(this)" value="删除"><input type="button" onclick="addLine()" value="增行"></td></tr>');
			}else{
                          ...
                          success : function(data) {
                          	...
                          	if(data.length>0){
	                          	...
	                          	for(var i=0;i<data.length;i++){
									...
									var text ='<tr id="'+id+'"><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="'+pid+'"></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="'+cid+'"></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="'+did+'" class="district"></select></td>'+'<td style="text-align: center; vertical-align: middle; ">'+data[i].DUTY_DEPT_NAME+'</td><td style="text-align: center; vertical-align: middle; display: none">'+data[i].DUTY_DEPT_ID+'</td><td style="text-align: center; vertical-align: middle; "><input type="button" onclick="del(this)"  value="删除"/><input type="button" onclick="addLine()" value="增行"/></td></tr>';
									$('#users').append(text);
									$("#"+id).distpicker({
		...
		function savaByQuery(t,_dataSourceCode,$div){
			...
			$.ajax({  
              url : "/system/project/saveProduct?json="+JSON.stringify(saveDate)+"&id="+ParentPKValue,  
              dataType : "json",
                          type : "GET", 
                          async: false, 
                          success : function(data) {
	...
	$("body").on("keyup", "", function (){
		...
		function addLine(){
			var id = "distpicker-"+i_count;
			var did = "district-"+i_count;
			var text ='<tr id="'+id+'"><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="province"></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="city"></select></td><td style="text-align: center; vertical-align: middle; "><select class="form-control" id="'+did+'" class="district"></select></td>'+'<td style="text-align: center; vertical-align: middle; "></td><td style="text-align: center; vertical-align: middle; display: none"></td><td style="text-align: center; vertical-align: middle; "><input type="button" onclick="del(this)"  value="删除"/><input type="button" onclick="addLine()" value="增行"/></td></tr>';
			$('#users').append(text);
