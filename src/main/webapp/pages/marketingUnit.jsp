...
function aChartInit(value){
	...
	aChart.setOption({
	 ...
	 $.ajax({
		url : '/system/buttonBase?cmd=button&buttonToken=query&dataSourceCode=VIEW_CONFIRM_INCOME_REPORT&token='+token+'&limit=9999&offset=0&_=1530779481604&CONFIRM_MONTH='+value,
		type : 'post',
		async : true,
		data : {},
		dataType : "json",
		success : function(data) {
			...
			if(rows.length < 1){
				...
				for(var i=0;i<rows.length;i++){
					legendData.push(rows[i].INCOME_OF_DEPT);
					seriesData.push({value:rows[i].CLAIM_BALANCE,name:rows[i].INCOME_OF_DEPT});
				}
			}
			aChart.setOption({
				legend: {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(rows.length < 1){
				...
				for(var i=0;i<rows.length;i++){
					xAxisData.push(rows[i].INCOME_OF_DEPT);
