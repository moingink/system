...
var code = '<%=request.getParameter("VCODE")%>';
...
window.onload = function(){
	...
	if($.isEmptyObject(fill) == true){
		$.ajax({  
              url : "/system/project/getProposalChild?proj_source_id="+ParentPKValue+"&code="+code+"&table="+pageCode,  
              dataType : "json",  
                          type : "GET",  
                          async: false, 
                          success : function(data) {
	                          ...
	                          success : function(data) {
		                      	if(data.length>0){
				            		bill_statue = data[0].BILL_STATUS;
				           			console.log("建议书立项成本="+data[0].PROJECT_COST);
				           			console.log(typeof(data[0].PROJECT_COST));
