...
<body>
<button onclick="saveProjectOra()" id="ins_or_up_buttontoken">保存</button>
<form id="form">
<table border="1" width="80%" id="table" class="table table-hover">
<thead>
	<tr class="info">
	<th>序号</th>
	<th>职能组</th>
	<th style="width: 70px;">人员类型</th>
	<th>姓名</th>
...
window.onload = function(){
	...
	if($.isEmptyObject(fill) == true){
		$.ajax({  
            url : "/system/projectHistory/getOrag?proj_released_id="+ParentPKValue+"&version_history="+version_history,
            dataType : "json",  
            type : "GET",  
            async: false,
            success : function(data) {
     	...
     	//如果有数据则回显数据
  		...
  		for(var i=0;i<fill.length;i++){
			...
			t4.innerHTML='<input type="text" style="width:100%" value="'+fill[i].NAME+'" />';
			...
			t6.innerHTML='<input type="text" style="width:100%" value="'+fill[i].EMAIL+'"/>';
			t7.innerHTML='<input type="text" style="width:100%" value="'+fill[i].TEL+'"/>';
...
var selectvar = '<select id="UrbanDepNo0" name="UrbanDepNo" style="width:100%">'+
 ...
 '<option value="6">客户验收上线</option></select>';
	function add(){
		...
		t7.innerHTML='<input type="text" style="width:100%" />';
...
function jsonValue(){
	...
	for(var i=0;i<l;i++){
 	...
 	for(var k=0; k*11 < data.length;k++){
 		var j = {};
 		j.PROJ_SOURCE_ID = ParentPKValue;
 		if(buttonToken=="deleteForProJect"){
		  		j.ID = data[k*11];
 		}
  		j.FUNCTIONAL_GROUP = data[k*11+1];
	  	j.PERSON_TYPE = data[k*11+2];
	  	j.NAME = data[k*11+3];
	  	j.ROLE = data[k*11+4];
	  	j.EMAIL = data[k*11+5];
	  	j.TEL = data[k*11+6];
