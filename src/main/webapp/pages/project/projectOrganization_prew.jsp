...
<div class="panel-body">
	...
	<div class="col-md-12 previewtitle">项目组织架构</div>
		<form id="form">
...
window.onload = function(){
	...
	if($.isEmptyObject(fill) == true){
		$.ajax({  
     	...
     	//如果有数据则回显数据
  		...
  		for(var i=0;i<fill.length;i++){
	        ...
	        t2.innerHTML='<select id="'+id+'" class="form-control" name="UrbanDepNo" style="width:100%"><option value="1">项目管理委员会</option><option value="2">项目核心管理团队</option><option value="3">资源环境准备组</option><option value="4">系统设计组</option><option value="5">应用开发部署组</option><option value="6">系统集成组</option><option value="7">质量保证团队</option></select>';
			t3.innerHTML='<select id="UrbanDepNo0" name="UrbanDepNo" class="form-control"><option value="1">内部人员</option><option value="2">外部人员</option></select>';
			t4.innerHTML='<input type="text" class="form-control" value="'+fill[i].NAME+'" />';
			var ida = "role"+i;
			var role_value = fill[i].ROLE;
			t5.innerHTML='<select id="'+ida+'" name="UrbanDepNo" class="form-control"><option value="1">领导</option><option value="2">总、副总</option><option value="3">分管领导</option><option value="4">销售总监</option><option value="5">项目总监</option><option value="6">产品总监</option><option value="7">项目经理</option><option value="8">客户经理</option><option value="9">产品方案经理</option><option value="10">运营经理</option><option value="11">维护经理</option><option value="12">现场协调</option><option value="13">软件架构师</option><option value="14">系统架构师</option><option value="15">开发经理</option><option value="16">前台工程师</option><option value="17">后台工程师</option><option value="17">质量保障经理</option><option value="19">测试工程师</option></select>';
			t6.innerHTML='<input type="text" class="form-control" value="'+fill[i].EMAIL+'"/>';
			t7.innerHTML='<input type="text" class="form-control" value="'+fill[i].TEL+'"/><input type="hidden"    value="'+fill[i].USER_ID+'"/>';
			//t8.innerHTML='<input type="text" style="width:100%" value="'+fill[i].CUST_NAME+'"/>';
			...
			//t10.innerHTML='<input type="text" style="width:100%" value="'+fill[i].CUST_EMAIL+'"/>';
			//t11.innerHTML='<input type="text" style="width:100%" value="'+fill[i].CUST_TEL+'"/>';
		 ...
		 $("#UrbanDepNo0").change(function(){
		 	if($(this).val()=='1'){
			 	$(".active").find("td:nth-child(4)>input").attr("readonly","true");
		 	}else{
			 	$(".active").find("td:nth-child(4)>input").removeAttr("readonly");
		 	}
		 });
		 
 		$("tr>td:nth-child(4)>input").click(function(){
 			if($(this).parent().parent().find("td:nth-child(3)>select").val()=='1'){
 				$("tr>td:nth-child(4)>input").removeAttr("id");
 				$(this).attr("id","USER_NAME");
 				$("tr>td:nth-child(7)>input:nth-child(2)").removeAttr("id");
 				$(this).parent().parent().find("td:nth-child(7)>input:nth-child(2)").attr("id","USER_ID");
 				$("tr>td:nth-child(6)>input").removeAttr("id");
 				$(this).parent().parent().find("td:nth-child(6)>input").attr("id","USER_EMAIL");
 				$(this).attr("readonly","true");
	 			checkReference(this,'REF(RM_USER,NAME:USER_NAME;EMAIL:USER_EMAIL;ID:USER_ID,0)','NAME','INSUP');
 			}else{
        ...
        if(keyWord == 1){//iframe
		 ...
		 hide(isHide);//隐藏及置灰
		 checkReference(null,'REF(RM_USER,NAME:USER_NAME;EMAIL:USER_EMAIL;ID:USER_ID,0)','NAME','INSUP');
} 
function ref_end1(){
...
var selectvar = '<select id="UrbanDepNo0" name="UrbanDepNo" style="width:100%">'+
 ...
 '<option value="6">客户验收上线</option></select>';
	function add(){
		...
		t1.innerHTML='<input type="text" style="width:100%;display: none" value="" />';
		t2.innerHTML='<select id="s1" name="UrbanDepNo" style="width:100%"><option value="1">项目管理委员会</option><option value="2">项目核心管理团队</option><option value="3">资源环境准备组</option><option value="4">系统设计组</option><option value="5">应用开发部署组</option><option value="6">系统集成组</option><option value="7">质量保证团队</option></select>';
		t3.innerHTML='<select id="UrbanDepNo0" name="UrbanDepNo" style="width:100%"><option value="1">内部人员</option><option value="2">外部人员</option></select>';
		t4.innerHTML='<input type="text" style="width:100%" />';
		t5.innerHTML='<select id="s3" name="UrbanDepNo" style="width:100%"><option value="1">领导</option><option value="2">总、副总</option><option value="3">分管领导</option><option value="4">销售总监</option><option value="5">项目总监</option><option value="6">产品总监</option><option value="7">项目经理</option><option value="8">客户经理</option><option value="9">产品方案经理</option><option value="10">运营经理</option><option value="11">维护经理</option><option value="12">现场协调</option><option value="13">软件架构师</option><option value="14">系统架构师</option><option value="15">开发经理</option><option value="16">前台工程师</option><option value="17">后台工程师</option><option value="17">质量保障经理</option><option value="19">测试工程师</option></select>';
		t6.innerHTML='<input type="text" style="width:100%" />';
		t7.innerHTML='<input type="text" style="width:100%" /><input type="hidden"    value=""/>';
		t8.innerHTML='<input type="text" style="width:100%" />';
		t9.innerHTML='<select id="s3" name="UrbanDepNo" style="width:100%"><option value="1">领导</option><option value="2">总、副总</option><option value="3">分管领导</option><option value="4">销售总监</option><option value="5">项目总监</option><option value="6">产品总监</option><option value="7">项目经理</option><option value="8">客户经理</option><option value="9">产品方案经理</option><option value="10">运营经理</option><option value="11">维护经理</option><option value="12">现场协调</option><option value="13">软件架构师</option><option value="14">系统架构师</option><option value="15">开发经理</option><option value="16">前台工程师</option><option value="17">后台工程师</option><option value="17">质量保障经理</option><option value="19">测试工程师</option></select>';
		t10.innerHTML='<input type="text" style="width:100%" />';
		ta.innerHTML='<input type="text" style="width:100%" />';
	...
	for(var i=0;i<l;i++){
 	...
 	for(var k=0; k*8 < data.length;k++){
 		...
 		if(buttonToken=="deleteForProJect"){
	  	...
	  	j.NAME = data[k*8+3];
	  	j.ROLE = data[k*8+4];
	  	if(checkEmail(data[k*8+5])==false){
	  		alert('请填写正确邮箱');
	  		return false;
	  	}
	  	j.EMAIL = data[k*8+5];
	  	if(checkPhone(data[k*8+6])==false){
	  		alert('请填写正确手机号');
	  		return false;
	  	}
	  	j.TEL = data[k*8+6];
	  ...
	  /*	j.CUST_NAME = data[k*11+7];
	  	j.CUST_ROLE = data[k*11+8];
	  	j.CUST_EMAIL = data[k*11+9];
	  	j.CUST_TEL = data[k*11+10];*/
	  	j.DR='0';
	  	j.PROJ_TYPE = '0';
  		saveDate.push(j);
  	}
