...
<body>
<button onclick="saveRisk()" id="ins_or_up_buttontoken">保存</button>
<form id="form">
...
window.onload = function(){
	...
	if($.isEmptyObject(fill) == true){
		$.ajax({  
                url : "/system/projectHistory/risk?proj_released_id="+ParentPKValue+"&version_history="+version_history,  
                dataType : "json",  
                            type : "GET",  
                            async: false, 
                            success : function(data) {
       ...
       if($.isEmptyObject(fill) == false){
           ...
           for(var i=0;i<fill.length;i++){
				...
				t1.innerHTML ='<input type="text" style="width:98%" value="'+fill[i].INFORMATION+'"/>';
				var id= "select"+i;
				var fxstatue = fill[i].INFLUENCE_LEVEL;
				t2.innerHTML='<select id="'+id+'" name="UrbanDepNo" style="width:90%"><option value="1">高</option><option value="2">中</option><option value="3">低</option></select>';
				//t3.innerHTML='<input type="text" style="width:98%" value="'+fill[i].OCCURENCE_TIME+'"/>';
				t3.innerHTML='<div class="input-group date" id="date1"><input type="text" class="form-control" value="'+fill[i].OCCURENCE_TIME+'" /><span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span></div>';
				var ida = "statue"+i;
				t4.innerHTML='<select id="'+ida+'" name="UrbanDepNo" style="width:90%"><option value="1">是</option><option value="2">否</option></select>';
				var statue = fill[i].INTERNAL_SOLUTION;
				
				t5.innerHTML='<input type="text" style="width:98%" value="'+fill[i].SOLUTION_MEASURES+'"/>';
				//t6.innerHTML='<input type="text" style="width:98%" value="'+fill[i].DEADLINE+'"/>';
				t6.innerHTML='<div class="input-group date" id="date1"><input type="text" class="form-control" value="'+fill[i].DEADLINE+'" /><span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span></div>';
				var idb = "zt"+i;
				t7.innerHTML='<select id="'+idb+'" name="riskStatue" style="width:90%"><option value="0"></option><option value="1">开启</option><option value="2">关闭</option><option value="3">风险转问题</option></select>';
...
function add(){
	...
	t3.innerHTML='<div class="input-group date" id="date1"><input type="text" class="form-control" value="" /><span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span></div>';
	...
	t6.innerHTML='<div class="input-group date" id="date1"><input type="text" class="form-control" value="" /><span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span></div>';
