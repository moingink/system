...
<script type="text/javascript">
		...
		$(function(){
			...
			//	height:250,
				...
				singleSelect:true,
				idField:'itemid',
				url:'/system/cuma?cmd=find_car_tain_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=7',
				columns:[[
		...
		function deleterow(target){
			...
			$.ajax({
			type: "POST",
			url:"/system/cuma?cmd=delete_box_tain_message&id="+map+"&cus_id="+cus_id+"&is_no=4",
		...
		function saverow(target){
            ...
            $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_tain_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=8&page=1&rows=10&car_map="+map,
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
...
function isPoneAvailables($poneInput) {
...
function costmessage(message) {
	if(!isPoneAvailables(message)){
