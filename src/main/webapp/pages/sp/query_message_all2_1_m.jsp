...
<script type="text/javascript">
	function save_car_message(){
	  ...
	  //alert(car_map);
	   ...
	   $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=6&page=1&rows=10&car_map="+car_map,		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
...
<body>	
    ...
    <button onclick="sure_message()" class="btn btn-success">确定 </button> <label>Tip：请先选择客户车型，再添加车型服务内容。</label>                  
                         
	 <table id="tt_message1" class="easyui-datagrid" style="width: 100%"
			data-options="singleSelect:false,rownumbers:true,fitColumns:true,pagination:true,collapsible:true,url:'/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=5',method:'post'">
		<thead>
			<tr>
