...
$(function(){
 ...
 function remove_hidden(){
	...
	function yzxx(){
	    ...
	    if($("#j_1").val()=="" ){
	   		var errorMsg1 = '<small class="help-block" style="color:#a94442;">联系人 必填不能为空!</small>';
		    $('#errors').append(errorMsg1);		    
	   	}
	   	if($("#j_2").val()==""){
	    	var errorMsg2 = '<small class="help-block" style="color:#a94442;">联系人职务 必填不能为空!</small>';
		    $('#errors').append(errorMsg2);		   
		}
		if($("#j_3").val()==""){
	    	var errorMsg3 = '<small class="help-block" style="color:#a94442;">列入原因 必填不能为空!</small>';
		    $('#errors').append(errorMsg3);		  
		}
		if($("#j_4").val()==null){
	    	var errorMsg4 = '<small class="help-block" style="color:#a94442;">职能角色 必填不能为空!</small>';
		    $('#errors').append(errorMsg4);		  
		}
		if($("#j_5").val()==""){
	    	var errorMsg5 = '<small class="help-block" style="color:#a94442;">角色诉求  必填不能为空!</small>';
		    $('#errors').append(errorMsg5);    
	    }
	    if($("#j_6").val()==""){
	    	var errorMsg6 = '<small class="help-block" style="color:#a94442;">联系方式  必填不能为空!</small>';
		    $('#errors').append(errorMsg6);    
	    }	
	    if($("#j_7").val()==""){
	    	var errorMsg7 = '<small class="help-block" style="color:#a94442;">备注信息  必填不能为空!</small>';
		    $('#errors').append(errorMsg7);    
	    }
	...
	function save_car_message(){
	   ...
	   if(j8==""){j9="0";}
	   var car_map=[car_id,j1,j2,j3,j4,j5,j6,j7,j8,cus_id,j9];
       ...
       if($("#j_1").val()=="" && $("#j_2").val()=="" && $("#j_3").val()=="" && $("#j_4").val()=="" && $("#j_5").val()=="" && $("#j_6").val()=="" && $("#j_7").val()==""){
	       	var errorMsg1 = '<small class="help-block" style="color:#a94442;">联系人 必填不能为空!</small>';
		    $('#errors').append(errorMsg1);
	   ...
	   }else if($("#j_1").val()=="" || $("#j_2").val()=="" || $("#j_3").val()=="" || $("#j_4").val()==null || $("#j_5").val()=="" || $("#j_6").val()=="" || $("#j_7").val()==""){

	         var asd = Array;
	         asd=[1,2,3,4,5,6,7,];
	         for(var i=0;i<asd.length;asd++){
	           	yzxx();
	         }
	   }
	   else{ 
	    ...
	    $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=10&page=1&rows=10&car_map="+car_map,		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
 ...
 function update_messages(){
	    ...
	    $('#errors').find(".help-block").remove();
	  	...
	  	if(row==""){
	       ...
	       save_update=[car_id,j1,j2,j3,j4,j5,j6,j7,j8,cus_id,j9];
	      }
	 ...
	 function save_car_message1(){
        ...
        $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=10&page=1&rows=10&car_map="+save_update,		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
	  	...
	  	var row = $('#tt_message').datagrid('getSelections');
	  	  ...
	  	  if(row==""){
	  	   ...
	  	   for(var i=0;i<row.length;i++){
	  	    ...
	  	    $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=delete_box_message&id="+id+"&cus_id="+car_id+"&is_no=2",		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
...
<body>	
         ...
         <tr >
	         ...
	         <td class="col-md-3">
	         	<div class="form-group">
                   ...
                   <div class="col-md-7">
                   	<select id="j_4" class="form-control">
         ...
         <tr>
         	...
         	<td class="col-md-3">
         		<div class="form-group">
                   ...
                   <div class="col-md-7">
                   	<input id="j_7" type="text" placeholder="请描述联系人xxxx信息" class="form-control" data-toggle="tooltip" data-original-title=" 备注信息:"/>
                   </div>
                </div>
         	</td>
            <td class="col-md-3">
	...
	<table id="tt_message" class="easyui-datagrid" style="width: 100%"
			data-options="pageList:[10,20,50],singleSelect:false,rownumbers:true,fitColumns:true,pagination:true,collapsible:true,url:'/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=9',method:'post'">
		<thead>
			<tr>
