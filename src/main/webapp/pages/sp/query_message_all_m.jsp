...
<script type="text/javascript">
 ...
 function update_messages(){
	 ...
	 function save_car_message1(){
        ...
        $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=2&page=1&rows=10&car_map="+save_update,		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
	  	...
	  	var row = $('#tt_message').datagrid('getSelections');
	  	  ...
	  	  if(row==""){
	  	   ...
	  	   for(var i=0;i<row.length;i++){
	  	    ...
	  	    $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=delete_box_message&id="+id+"&cus_id="+car_id+"&is_no=0",		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
    ...
    $.ajax({
			type: "POST",
			url:"/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=0&page=1&rows=10",		
			dataType: "json",
			async: false,
			cache: false,
			contentType: false,
...
<body>	
	...
	<table id="tt_message" class="easyui-datagrid" style="width: 100%"
			data-options="singleSelect:false,rownumbers:true,fitColumns:true,pagination:true,collapsible:true,url:'/system/cuma?cmd=find_car_message&id=<%=request.getParameter("ParentPKValue")%>&is_no=0',method:'post'">
		<thead>
			<tr>
